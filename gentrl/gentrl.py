...
class GENTRL(nn.Module):
    ...
    def __init__(self, enc, dec, latent_descr, feature_descr, tt_int=40,
                 tt_type='usual', beta=0.01, gamma=0.1, device=torch.device("cpu")):
        ...
        self.lp = LP(distr_descr=self.latent_descr + self.feature_descr,
                     tt_int=self.tt_int, tt_type=self.tt_type).to(self.device)
    ...
    def get_elbo(self, x, y):
        ...
        # y = y.to("cpu")
        # latvar_samples = latvar_samples.to("cpu")

        zy = torch.cat([latvar_samples, y], dim=1)
        # zy = zy.to(self.device)
        ...
        # normal_distr_hentropies = normal_distr_hentropies.to("cpu")
        # log_p_zy = log_p_zy.to("cpu")
    ...
    def train_as_vaelp(self, train_loader, num_epochs=100,
    ...
    def train_as_rl(self,
                    reward_fn,
                    num_iterations=400, verbose_step=50,
                    batch_size=200,
                    cond_lb=-2, cond_rb=0,
        ...
        while cur_iteration < num_iterations:
            ...
            peptides = self.dec.sample(50, z, argmax=False)
            zc = torch.zeros(z.shape[0], 1).to(z.device)
            conc_zy = torch.cat([z, zc], dim=1)
            log_probs = self.lp.log_prob(conc_zy, marg=50 * [False] + [True])
            log_probs += self.dec.weighted_forward(peptides, z)
            r_list = [reward_fn(s) for s in peptides]
            ...
            valid_sm = [p for p in peptides if p is not None]
            cur_stats = {'mean_reward': sum(r_list) / len(peptides),
                         'valid_perc': len(valid_sm) / len(peptides)}
